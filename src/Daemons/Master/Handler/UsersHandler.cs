...
namespace Minerva
{
    partial class SyncHandler
    {
        ...
        public void SetUsersData(Dictionary<ulong, int> users)
        {
            this.users = users;
        }

        public bool UserExists(ulong magic)
        {
            lock (users)
            {
                return users.ContainsKey(magic);
            }
        }

        public void AddUser(ulong magic, int id)
        {
            lock (users)
            {
                users.Add(magic, id);
